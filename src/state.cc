...
Pool* State::LookupPool(const string& pool_name) {
  mblock_map<mblock_string, Pool*>::type::iterator i =
      pools_.find(pool_name.c_str());
  if (i == pools_.end())
    return NULL;
  return i->second;
}
...
void State::Dump() {
  ...
  if (!pools_.empty()) {
    ...
    for (mblock_map<mblock_string, Pool*>::type::const_iterator it =
             pools_.begin();
         it != pools_.end(); ++it) {
